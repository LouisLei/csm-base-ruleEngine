...
public class RuleController extends BasicController {
	 ...
	 * @return
	 */
	@ApiOperation("新增或者修改一个指定的rule信息："
			+ "新增rule信息时的规则逻辑请参考createRule的规则。"
			+ "修改信息时只能修改（只允许修改）以下属性"
	...
	@RequestMapping(value = "/bind/{policyId}/{ruleIds}", method = RequestMethod.POST)
	 public ExecuteHttpResponse bind(@PathVariable("policyId") String policyId , @PathVariable("ruleIds") String[] ruleIds) {
		return null;
	}
	...
	@RequestMapping(value = "/unbind/{policyId}/{ruleIds}", method = RequestMethod.POST)
	 public ExecuteHttpResponse unbind(@PathVariable("policyId") String policyId , @PathVariable("ruleIds") String[] ruleIds) {
		return null;
	}
	 ...
	 * @return
	 */
	@ApiOperation("查询当前系统中可用的所有rule信息。这些信息将按照被创建的时间倒序排列"
			+ "。注意，目前还没有分谢方法，后续该系统真正被使用了，在进行增加")
	 ...
	 * @return
	 */
	@ApiOperation("这些rule信息将依据绑定的policyId被查询出来，并且按照执行顺序依次排列。"
			+ "如果当前policy没有绑定任何编号信息，则返回一个空集合")
