...
public class PolicyServiceImp implements PolicyService {
  ...
  /* (non-Javadoc)
   * @see com.cheshangma.platform.ruleEngine.core.service.PolicyService#save(com.cheshangma.platform.ruleEngine.module.PolicyModel)
   */
  @Override
  @Transactional
  public PolicyModel save(PolicyModel model) {
    ...
    /*
     * 注意，保存只是保存policy的基本信息，和一同新增的元数据信息
     * 但是并不包括可能的rule步骤信息，后者这些步骤信息是分步进行绑定的
     * 之前都已经做过参数正确性判断了，这里不再进行赘述
     * */
    ...
    policyEntity.setScoreExpression(!StringUtils.isEmpty(model.getScoreExpression())?model.getScoreExpression():"");
    policyEntity.setScriptLanguage(model.getScriptLanguage() != null?model.getScriptLanguage():ScriptLanguageType.LANGUAGE_GROOVY);
    ...
    if(policy != null) {
        throw new PolicyAllreadyExistException("policy allready exist! policyId = " + model.getPolicyId());
    }
    // 插入基本信息
  ...
  /* (non-Javadoc)
   * @see com.cheshangma.platform.ruleEngine.core.service.PolicyService#update(com.cheshangma.platform.ruleEngine.module.PolicyModel)
   */
  @Override
  @Transactional
  public PolicyModel update(PolicyModel model) {
    /*
     * 注意。policy修改操作，包括了修改基本信息中的四个字段
     * 还包括了修改绑定的元数据
     * 当时和其关联的rule信息，并不通过这里进行修改，而在专门的bind方法中进行修改
     * */
    PolicyEntity policyEntity = new PolicyEntity();
    policyEntity.setId(model.getId());
    policyEntity.setMode(model.getMode());
    policyEntity.setPolicyId(model.getPolicyId());
    policyEntity.setScoreExpression(model.getScoreExpression() == null?"":model.getScoreExpression());
    policyEntity.setScriptLanguage(model.getScriptLanguage() == null?ScriptLanguageType.LANGUAGE_GROOVY:model.getScriptLanguage());
    policyEntity.setDescription(model.getDescription() == null?"":model.getDescription());
  ...
  /* (non-Javadoc)
   * @see com.cheshangma.platform.ruleEngine.core.service.PolicyService#findByPolicyId(java.lang.String)
   */
  @Override
  ...
  /* (non-Javadoc)
   * @see com.cheshangma.platform.ruleEngine.core.service.PolicyService#deleteByPolicyId(java.lang.String)
   */
  @Override
  @Transactional
